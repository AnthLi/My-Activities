...
sumX, sumY, sumZ, counter = 0, 0, 0, 0
...
def process(timestamp, values):
  ...
  # Calculate and print the averages once the counter is at least 100
  if counter >= 100:
    sumXAvg = sumX / counter
    sumYAvg = sumY / counter
    sumZAvg = sumZ / counter

    print('{}, {}, {}'.format(sumXAvg, sumYAvg, sumZAvg))
...
def authenticate(sock):
  '''
  Authenticates the user by performing a handshake with the data collection
  server.
...
try:
  ...
  while True:
    try:
      ...
      # if a connection timeout occurs, also ignore and try again. Use Ctrl-C to
      # stop but make sure the error is displayed so we know what's going on
