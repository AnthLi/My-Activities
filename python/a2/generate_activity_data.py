...
n_samples = 100
# number of feature dimensions; 2 for this example so that we can visualize
n_dim = 2

# for each activity, we specify the expected max, expected average and the
# standard deviation for simplicity the stddev is the same for both the max and
# average features.
stats = {
  "walking": {
    "expected_max": 7.0,
    "expected_average": 3.1,
    "stddev": 1.25,
    "samples": [],
    "format": "ro"
  },
...
# always call plt.figure() unless you want to plot points on an existing plot
plt.figure()
# handles to the plots, used to configure the legend
plt_handles = []
...
print "Average Precisions:", avgPrecA / 10, avgPrecB / 10, avgPrecC / 10
print "Average Recalls:", avgRecA / 10, avgRecB / 10, avgRecC / 10

# TODO: Then change the CV parameter shuffle to True and describe how the
# results change.
