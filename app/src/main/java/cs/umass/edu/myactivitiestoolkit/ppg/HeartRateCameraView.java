...
// the Camera class is deprecated, but we don't care for this assignment
public class HeartRateCameraView extends SurfaceView implements Callback, Camera.PreviewCallback {
  ...
  /** used for debugging purposes */ private static final String TAG = HeartRateCameraView.class.getName();
  ...
  public void onPreviewFrame(byte[] data, Camera camera) {
    ...
    for (int p : pixels) {
      meanRedValue += Color.red(p);
    }

    meanRedValue = meanRedValue / nPixels;
    PPGEvent event = new PPGEvent(meanRedValue, System.currentTimeMillis());
