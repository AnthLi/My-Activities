...
public abstract class SensorService extends Service implements ConnectionStateHandler {
  ...
  /** used for debugging purposes */ protected static final String TAG = SensorService.class.getName();
  ...
  protected Notification getNotification() {
    ...
    return new NotificationCompat.Builder(this).setContentTitle(getString(R.string.app_name)).setTicker(getString(R.string.app_name)).setContentText(getNotificationContentText()).setSmallIcon(getNotificationIconResourceID()).setOngoing(true).setVibrate(new long[]{0, 50, 150, 200}).setPriority(Notification.PRIORITY_MAX).addAction(R.drawable.ic_stop_white_24dp, "Stop Service", stopPendingIntent).setContentIntent(pendingIntent).build();
  }
